...
(function() {
    ...
    const settingsHandler = {
        ...
        set: (target, prop, value) => {
            ...
            else if(prop === "customGA4Patterns")
            {
                // Render the list of custom GA4 server-side endpoints
                let paramList = document.getElementById("customGA4-patterns");

                clearChildren(paramList);
                (value || []).forEach((param) => {
                    paramList.appendChild(createCustomGA4Param(param));
                });
            }
    ...
    document.getElementById("addCustomGA4").addEventListener("change", (event) => {
        event.preventDefault();

        let newParam = event.target.value.trim(),
            paramList = document.getElementById("customGA4-patterns");

        if (!Array.isArray(settings.customGA4Patterns)) {
            settings.customGA4Patterns = [];
        }

        // Basic validation: non-empty and not already in the list (case-insensitive)
        const duplicate = settings.customGA4Patterns.some(
            (p) => p.toLowerCase() === newParam.toLowerCase()
        );

        if (newParam !== "" && !duplicate) {
            paramList.appendChild(createCustomGA4Param(newParam));

            settings.customGA4Patterns.push(newParam);
            settingsProvider.save(settingsObj);

            tracker.track(["send", "event", "settings", "customGA4Patterns", `added: ${newParam}`]);
        }
    ...
    function createCustomGA4Param(param) {
        let text = createElement("span", {
                "text": param
            }),
            remove = createElement("span", {
        ...
        remove.addEventListener("click", (event) => {
            event.preventDefault();
            if (!Array.isArray(settingsObj.customGA4Patterns)) {
                settingsObj.customGA4Patterns = [];
            }
            settings.customGA4Patterns = settingsObj.customGA4Patterns = settingsObj.customGA4Patterns.filter(item => item !== param);
            settingsProvider.save(settingsObj);
            tracker.track(["send", "event", "settings", "customGA4Patterns", `removed: ${param}`]);
        });
