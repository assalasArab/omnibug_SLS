...
class OmnibugSettings {
    ...
    get defaults() {
        ...
        return {
            ...
            // Custom server-side GA4 endpoints (raw domain/path strings)
...
/**
 * Receive dataLayer events from the content script bridge and buffer them per-tab.
 */
chrome.runtime.onMessage.addListener((message, sender) => {
    if (!message || message.type !== "dataLayerEvent") { return; }
    if (!sender || !sender.tab || typeof sender.tab.id !== "number") { return; }

    const tabId = sender.tab.id;
    const event = {
        id: ++dataLayerEventSeq,
        type: "dataLayerEvent",
        layer: message.layer || "dataLayer",
        payload: message.payload,
        timestamp: message.timestamp || Date.now(),
        origin: message.origin || "push",
        url: message.url || ""
    };

    addDataLayerToHistory(tabId, event);

    // Push live to popup if connected
    if (popupPorts[tabId]) {
        try {
...
/**
 * Apply user-defined custom GA4 server-side patterns to the GA4 provider
 */
function applyCustomGA4Patterns(settings) {
    try {
        const providers = OmnibugProvider.getProviders();
        const ga4 = providers && providers["GOOGLEANALYTICS4"];
        if (ga4 && typeof ga4.setCustomPatterns === "function") {
            ga4.setCustomPatterns(Array.isArray(settings.customGA4Patterns) ? settings.customGA4Patterns : []);
        }
    } catch (e) {
        console.error("Failed to apply custom GA4 patterns", e);
    }
}
...
chrome.storage.onChanged.addListener((changes, storageType) => {
    ...
    settings.load().then((loadedSettings) => {
        applyCustomGA4Patterns(loadedSettings);
        sendSettingsToTabs(loadedSettings);
    });
});
...
/*
 Rolling buffer of captured requests per tab, for the popup view.
 Keyed by tabId. Each entry is an array of captured request events (finalData).
 Capped at MAX_HISTORY_PER_TAB and cleared when the tab navigates to a new URL.
 */
const requestHistory = {};
const MAX_HISTORY_PER_TAB = 100;

/*
 Rolling buffer of dataLayer push events per tab.
 Populated by the dataLayerBridge content script via chrome.runtime.sendMessage.
 Same cap and navigation-clear semantics as requestHistory.
 */
...
/*
 ...
 Separate from `tabs` (which holds devtools ports) so both can coexist.
 */
const popupPorts = {};
...
/**
 * Accept incoming connections from our devtools panels AND popup windows.
 *
 * - DevTools ports are named as the tabId directly (existing behavior).
 * - Popup ports are named "popup-<tabId>".
 */
chrome.runtime.onConnect.addListener((port) => {
    console.log(`Port ${port.name} connected`);

    // Popup connection: separate from devtools so they can coexist.
    if (port.name.indexOf("popup-") === 0) {
        const tabId = parseInt(port.name.substring("popup-".length), 10);
        if (isNaN(tabId)) {
            port.disconnect();
            return;
        }
        ...
        port.onDisconnect.addListener(() => {
            console.log(`Popup port for tab ${tabId} disconnected`);
            delete popupPorts[tabId];
        });

        // Load settings & push the current buffer to the popup immediately.
        const settings = new OmnibugSettings();
        settings.load().then((loadedSettings) => {
            applyCustomGA4Patterns(loadedSettings);
            try {
            ...
            // Make sure the provider regex is up to date too
            if (!(providerPattern instanceof RegExp)) {
                providerPattern = OmnibugProvider.getPattern(loadedSettings.providers);
            }
        });

        // Popup can send a "clear" message to wipe the history for its tab
    ...
    // DevTools connection (existing behavior)
...
chrome.webRequest.onHeadersReceived.addListener(
    (details) => {
        // Only relevant for the devtools panel (error rows). Popup doesn't track errors.
        if (!validDevtoolsProviderRequest(details) || details.statusCode < 400) { return; }
...
chrome.webRequest.onErrorOccurred.addListener(
    (details) => {
        // Only relevant for the devtools panel (error rows). Popup doesn't track errors.
        if (!validDevtoolsProviderRequest(details)) { return; }
...
/**
 * Clean up buffers when a tab is closed.
 */
...
/**
 * Append a captured request to the rolling history for its tab.
 * Caps per-tab history at MAX_HISTORY_PER_TAB.
 *
 * @param {number} tabId
 * @param {object} finalData
 */
function addToHistory(tabId, finalData) {
    if (tabId === -1) { return; }
    if (!requestHistory[tabId]) { requestHistory[tabId] = []; }
    requestHistory[tabId].push(finalData);
    if (requestHistory[tabId].length > MAX_HISTORY_PER_TAB) {
        requestHistory[tabId].splice(0, requestHistory[tabId].length - MAX_HISTORY_PER_TAB);
    }
}

/**
 * Append a dataLayer event to the per-tab buffer with capping.
 *
 * @param {number} tabId
 * @param {object} event
 */
function addDataLayerToHistory(tabId, event) {
    if (tabId === -1) { return; }
    if (!dataLayerHistory[tabId]) { dataLayerHistory[tabId] = []; }
    dataLayerHistory[tabId].push(event);
    if (dataLayerHistory[tabId].length > MAX_DATALAYER_PER_TAB) {
        dataLayerHistory[tabId].splice(0, dataLayerHistory[tabId].length - MAX_DATALAYER_PER_TAB);
    }
}
 ...
 * @returns {boolean}
 */
function isProviderRequest(details) {
...
/**
 * Original check used by error handlers: only relevant when devtools is open.
 *
 * @param details
 * @returns {boolean}
 */
function validDevtoolsProviderRequest(details) {
    return isProviderRequest(details) && tabHasOmnibugOpen(details.tabId);
}
...
function sendSettingsToTabs(settings) {
    ...
    applyCustomGA4Patterns(settings);
    ...
    // Also broadcast to open popups
    Object.values(popupPorts).forEach((popupPort) => {
        try {
